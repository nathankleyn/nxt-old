...
class Commands::Motor
  ...
  def start
    ...
    if @control_power
      mode |= NXT::REGULATED
      reg_mode = NXT::REGULATION_MODE_MOTOR_SPEED
    else
      ...
      if @duration[:degrees] or @duration[:seconds]
        case @action
          when :constant
            run_state = NXT::MOTOR_RUN_STATE_RUNNING
          when :ramp_up
            run_state = NXT::MOTOR_RUN_STATE_RAMPUP
          when :ramp_down
            run_state = NXT::MOTOR_RUN_STATE_RAMPDOWN
        end
      end
    end

    @nxt.set_output_state(
      NXT.const_get("MOTOR_#{@port.to_s.upcase}"),
      motor_power,
      mode,
      reg_mode,
      turn_ratio,
    ...
    if (@duration and @duration[:seconds]) or @wait
      if @duration and @duration[:seconds]
        ...
        until self.run_state == NXT::MOTOR_RUN_STATE_IDLE
          sleep(0.25)
        end
      end
      self.stop
