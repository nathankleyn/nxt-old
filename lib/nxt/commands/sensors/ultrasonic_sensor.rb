...
class Commands::UltrasonicSensor < Commands::Sensor
  ...
  def distance
    @nxt.ls_write(NXT.const_get("SENSOR_#{@port}"), Communication::Ultrasonic.read_measurement_byte(0))
  ...
  def set_mode
    ...
    @nxt.ls_write(NXT.const_get("SENSOR_#{@port}"), Communication::Ultrasonic.continuous_measurement_command)
  end
